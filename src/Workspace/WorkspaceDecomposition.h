...
class WorkspaceDecomposition : public stapl::sequential::graph<stapl::DIRECTED,
  ...
  public:
    ...
    const WorkspaceRegion& GetRegion(const size_t _i) const {
      return find_vertex(_i)->property();
		...
		const size_t FindRegion(const Point3d& _p);
    ...
    void Finalize();
		...
		/// Initialize Segment trees to expedite search of region for a workspace point
		void InitializeSegmentTree(const size_t _dim = 3);
  ...
  private:
    ...
    bool m_finalized{false};  ///< Is the decomposition finalized?
		...
		SegmentTrees<vertex_descriptor>* m_segmentTree{nullptr};		///< Segment Trees to expedite search of workspace point
    ///@}
};
