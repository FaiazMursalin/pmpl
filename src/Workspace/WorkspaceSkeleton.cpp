...
unordered_map<size_t,vector<pair<Point3d,Point3d>>> 
WorkspaceSkeleton::
FindEdgeRegions(GraphType::const_edge_iterator& _ei, WorkspaceDecomposition* _wd, const size_t _s)	{
	//Storage 
	typedef vector<pair<Point3d,Point3d>> PointPairs;
	unordered_map<size_t, PointPairs> regions;
	size_t startP, endP;
	size_t prevRegion, region;
	size_t invalidRegion = numeric_limits<size_t>::max();

	// Function for storing in the map region 
  static auto StoreRegion = [&](size_t& _r, Point3d& _p1, Point3d& _p2) -> void {
  	auto it = regions.find(_r);
		if(it == regions.end())	
			regions.insert(make_pair(_r,PointPairs(1,make_pair(_p1,_p2))));
		else
			it->second.emplace_back(make_pair(_p1,_p2));
  };

	// Initialize
	startP = endP = 0;
	// Get the region where the source reside
	prevRegion = _s;

	// Get the points that represent the edge
	auto points = _ei->property();
	// For all the points find the region they belong to 
	size_t i = 0;
	bool inObstacle = false;
	for(auto pit = points.cbegin(); pit != points.cend(); ++pit, ++i)	{
		region = invalidRegion;
		// If the previous point was in a region 
		if(!inObstacle && prevRegion < invalidRegion)	{
			// Point is not in the previous region
			if(_wd->GetRegion(prevRegion).PointIsOutside(*pit))	{
				auto vit = _wd->find_vertex(prevRegion);
				// For every adjacent edge - find the neighbor region
				for(auto eit = vit->begin(); eit != vit->end(); ++eit)	{
					auto neighbor = eit->target();
					if(!_wd->GetRegion(neighbor).PointIsOutside(*pit))	{
						region = neighbor;
						break;
					}
				}
		...
		// Couldn't find the region yet - find it brute force way through segment trees
		if(region == invalidRegion)	
			region = _wd->FindRegion(*pit);
		// A region is found
		if(region < invalidRegion)	{
			// If it is a transition point between regions or obstacle space
			if(inObstacle || region != prevRegion)	{
				// set the previous set of start and end points
				if(!inObstacle)
					StoreRegion(region,points[startP],points[endP]);
				startP = i;
				prevRegion = region;
			}
			// reset end point
