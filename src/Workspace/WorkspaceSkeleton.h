...
class WorkspaceSkeleton {
  ...
  public:
    ...
    void IncrementSuccess(Boundary* _region);
		...
		/// Get the intersecting points given a decomposition
		/// @param _ei Edge iterator
		/// @param _wd Workspace Decomposition
		/// @param _s  Region index of start vertex (default value: not found region)
		virtual unordered_map<size_t,vector<pair<Point3d,Point3d>>> 
			FindEdgeRegions(GraphType::const_edge_iterator& _ei, 
											WorkspaceDecomposition* _wd, 
											const size_t _s = numeric_limits<size_t>::max());
