...
class PQP : public CollisionDetectionMethod {
  ...
  public:
    ...
    virtual bool IsInCollision(const Body* const _body1,
        const Body* const _body2, CDInfo& _cdInfo, bool _cache=true) override;
...
class PQPSolid : public PQP {
  ...
  public:
    ...
    /// Shoot a ray outward from a reference point and along a given direction
    /// to determine if it intersect within a given body and storing the 
    /// distance from _pt to nearest point of intersection
    /// @param[in] _pt The reference point of interest.
    /// @param[in] _dir The direction of the ray.
    /// @param[in] _body The body to check against.
    /// @return True if ray intersect with the _body
    bool RayIntersection(const Vector3d& _pt, const Vector3d& _dir, 
