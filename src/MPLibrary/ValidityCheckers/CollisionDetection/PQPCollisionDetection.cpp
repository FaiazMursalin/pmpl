...
IsInCollision(const Body* const _body1, const Body* const _body2,
  ...
  if(!collision){
    collision = IsInsideObstacle(_body1->GetPolyhedron().m_vertexList[0] * _body1->GetWorldTransformation(),
                                 _body2);
  }
  return collision;
}
...
bool 
PQPSolid::
RayIntersection(const Vector3d& _pt, const Vector3d& _dir, 
       const Body* const _body, CDInfo& _cdInfo) {

  // Build a ray model
  PQP_Model* ray = new PQP_Model();
  PQP_REAL zero[3] = {0, 0, 0};
  PQP_REAL infnty[3] = {1e10*_dir[0], 1e10*_dir[1], 1e10*_dir[2]};

  ray->BeginModel();
  ray->AddTri(zero, zero, infnty, 0);
  ray->EndModel();
  ...
  for(int i = 0; i < result.NumPairs(); ++i) {
    ...
    // Skip collisions against triangles whose normals are perpendicular to the
    // ray: these are scrapes and don't affect inside/outside-ness.
    if(abs(n*_dir) < tolerance) continue;
    // Distance of the closest point in the triangle from the point
    // closest pt on plane P': P0 + su 
    // where u is the directional vector of the line
    // s is n.(v - P0)/n.u
    // |P' - P0| = P0 + su - P0 = |su|
    const double alpha = ((v - _pt) * n) / (_dir * n);
    if(abs(alpha) < distance) {
       distance = abs(alpha);
       _cdInfo.m_minDist = distance;
       _cdInfo.m_objectPoint = _pt + (alpha * _dir);
       _cdInfo.m_rapidContactID2 = result.Id2(i);
    }
  } 
...
PQPSolid::
RayIntersection(const Vector3d& _pt, const Vector3d& _dir, 
       const Environment* _env, CDInfo& _cdInfo) {
  size_t nMulti = _env->NumObstacles();
  double distance = numeric_limits<double>::max();
  bool foundSpoke = false;
  // Check ray intersection with every multibody and find the minimum distance one
  for(size_t i = 0; i < nMulti; ++i) {
    CDInfo cdInfo(true);
    bool status = RayIntersection(_pt, _dir, _env->GetObstacle(i)->GetFixedBody(0), cdInfo);
    if(status && distance > cdInfo.m_minDist){
      foundSpoke = true;
      distance = cdInfo.m_minDist;
      _cdInfo.m_objectPoint = cdInfo.m_objectPoint;
      _cdInfo.m_rapidContactID2 = cdInfo.m_rapidContactID2;
    }
  }
  // Check against boundary
  auto boundary = _env->GetBoundary();
  double bndryDistance = boundary->GetDirectedClearance(_pt, _dir);
  if(bndryDistance >= 0 && bndryDistance < distance){
    distance = bndryDistance;
    _cdInfo.m_objectPoint = _pt + distance * _dir;
    foundSpoke = true;
  }
  if(foundSpoke)
    _cdInfo.m_minDist = distance;
