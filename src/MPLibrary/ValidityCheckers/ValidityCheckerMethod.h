...
class ValidityCheckerMethod : public MPBaseObject<MPTraits> {
  ...
  public:
    ...
    /// @endcode
    bool IsValid(CfgType& _cfg, CDInfo& _cdInfo, const string& _callName, bool _stat=true) {
      bool result = false;
      if(m_validity)
        result = IsValidImpl(_cfg, _cdInfo, _callName);
      else
        result = !IsValidImpl(_cfg, _cdInfo, _callName);
      return result;
    }
    ...
    virtual bool IsInsideObstacle(const CfgType& _cfg) {
      //need to check the value is negative or not check
      // cerr << "error: IsInsideObstacle() not defined." << endl;
      // return false;
      auto vc = this->GetValidityChecker("rapid");
      bool valid = vc->IsInsideObstacle(_cfg);
      return valid;
