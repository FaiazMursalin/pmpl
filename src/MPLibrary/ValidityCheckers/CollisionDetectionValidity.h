...
class CollisionDetectionValidity : public ValidityCheckerMethod<MPTraits> {
  ...
  public:
    ...
    bool GetIgnoreSelfCollision() { return m_ignoreSelfCollision; }
    size_t GetAdjacentLinks() { return m_ignoreIAdjacentLinks; }
  ...
  private:
    ...
    bool m_delaySelfCollision;            ///< Delay self collision till end
...
CollisionDetectionValidity(CollisionDetectionMethod* _cdMethod,
    ...
    m_ignoreSelfCollision(_ignoreSelfCollision), m_delaySelfCollision(false),
...
CollisionDetectionValidity(XMLNode& _node) :
  ...
  m_delaySelfCollision = _node.Read("delaySelfCollision", false, false,
      "Delay self collision");
...
IsValidImpl(CfgType& _cfg, CDInfo& _cdInfo, const string& _callName) {
  ...
  string clockName = this->GetNameAndLabel()+"::ValidityCheck";
  string clockObst = this->GetNameAndLabel()+"::ObstCheck";
...
IsInCollision(CDInfo& _cdInfo, const CfgType& _cfg, const string& _callName) {
  ...
  if(!m_delaySelfCollision && !m_ignoreSelfCollision 
     && multiBody->NumFreeBody() > 1 &&
  ...
  for(size_t i = 0; i < numObst; ++i) {
    ...
    if(coll && !_cdInfo.m_retAllInfo){
      if(!_cdInfo.m_retAllInfo) stat->StopClock(clockObst);
      return true;
    }
  }
  if(!_cdInfo.m_retAllInfo) stat->StopClock(clockObst);
...
IsInSelfCollision(CDInfo& _cdInfo, ActiveMultiBody* _rob,
  ...
  if(!_cdInfo.m_retAllInfo) {
    stat->IncNumCollDetCalls(m_cdMethod->GetName(), _callName);
