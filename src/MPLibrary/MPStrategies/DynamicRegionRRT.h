...
class DynamicRegionRRT : public BasicRRTStrategy<MPTraits> {
  ...
  public:
    ...
    DynamicRegionRRT(string _dm = "euclidean", string _nf = "Nearest",
        ...
        size_t _numDirections = 1, size_t _maxTrial = 3, string _inputSkeletonFilename = "");
  ...
  private:
    ...
    string m_inputSkeletonFilename; ///< Input skeleton to initialize map
...
ReadSkeleton()	{
  // Read the file
  ifstream input(m_inputSkeletonFilename);
  WorkspaceSkeleton::GraphType g;
...
DynamicRegionRRT(string _dm, string _nf, string _vc, string _nc, string _ex,
    ...
    size_t _maxTrial, string _inputSkeletonFilename) :
...
DynamicRegionRRT(XMLNode& _node) : BasicRRTStrategy<MPTraits>(_node) {
  ...
  m_biasing = _node.Read("biasing", false, false,
	...
	m_inputSkeletonFilename = _node.Read("inputSkeleton", false, "",
      "filename of skeleton to start from");
}
...
Initialize() {
  ...
  if(m_inputSkeletonFilename.empty())	{
    //Embed ReebGraph
    delete m_reebGraph;
	...
	cout<<"Robot Radius"<<robotRadius<<endl;
	cout<<"Before pruning"<<m_skeleton.GetGraph().get_num_edges()<<endl;
	// Clearance based filtration - Uncomment
	ClearanceFilteredSkeleton(robotRadius, this, &m_skeleton, false);
	cout<<"After pruning"<<m_skeleton.GetGraph().get_num_edges()<<endl;
