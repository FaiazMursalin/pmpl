...
UniformObstacleBasedSampler(XMLNode& _node) : SamplerMethod<MPTraits>(_node) {
  ...
  m_keepCollide = _node.Read("keepCollide", true, false,
      "Keep collision cfg");
...
Sampler(CfgType& _cfg, const Boundary* const _boundary,
  ...
  for(int i = 1; i < nTicks; i++) {
    ...
    if(tempFree == tickFree) {
      ...
      generated = true;
      if(((!m_keepCollide and tempFree) or (m_keepCollide and !tempFree)) and temp.InBounds(_boundary)) {
        _result.push_back(temp);
        tempFree = tickFree;
        temp = tick;
      }
      else if(((!m_keepCollide and tickFree) or (m_keepCollide and !tickFree)) and tick.InBounds(_boundary)) { //tickFree
