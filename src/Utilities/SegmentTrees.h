...
class SegmentTrees {
  public:
    ...
    /// Finds the Enclosing Boundaries
    /// @param _p Extreme points of the query bounding box
    size_t FindEnclosingBoundaries(pair<Point3d,Point3d>& _p);
...
void SegmentTrees<PropertyType>::BuildSegmentTrees() {
  ...
  if(m_dimension == 2) {
    ...
    for(size_t i = 0; i < m_boundaries.size(); i++) {
      ...
      auto x = b->GetRange(0);
      if(x.Length() < numeric_limits<float>::epsilon()) {
        x.min -= numeric_limits<float>::epsilon();
        x.max += numeric_limits<float>::epsilon();
      }
      auto y = b->GetRange(1); 
      if(y.Length() < numeric_limits<float>::epsilon()) {
        y.min -= numeric_limits<float>::epsilon();
        y.max += numeric_limits<float>::epsilon();
      } 
...
FindEnclosingBoundaries(const Point3d& _p, double _e) {
  pair<Point3d, Point3d> p;
  if(m_dimension == 2)
    p = make_pair(_p - Vector3d(_e,_e,0), _p + Vector3d(_e,_e,0)); 
  else
    p = make_pair(_p - Vector3d(_e,_e,_e), _p + Vector3d(_e,_e,_e)); 
  return FindEnclosingBoundaries(p);
}

template<typename PropertyType>
size_t 
SegmentTrees<PropertyType>::
FindEnclosingBoundaries(pair<Point3d,Point3d>& _p) {
  ...
  if(m_dimension == 2) {
    ...
    auto p = Interval2(Pure_interval2(Key2(_p.first[0],_p.first[1]), 
          Key2(_p.second[0],_p.second[1])),m_boundaries.size());
    ...
    auto p = Interval3(Pure_interval3(Key3(_p.first[0],_p.first[1],_p.first[2]), 
          Key3(_p.second[0],_p.second[1],_p.second[2])),m_boundaries.size());
