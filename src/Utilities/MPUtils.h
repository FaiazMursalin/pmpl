...
/// Polyline simplification based on Ramer Douglas Peucker algorithm
template<typename InputIterator>
vector<bool> LineSimplification(double _epsilon, InputIterator _start, InputIterator _end) {
	static auto DistanceToLine = [&](Point3d& _p0, Point3d& _p1, Point3d& _p2) -> double {
		auto v = (_p2 - _p1);
		return ((v % (_p1 - _p0)).norm())/(v.norm());
	};

	/// Function for the RDP computation
	function<void(double, InputIterator, InputIterator, vector<bool>::iterator)> 
	RamerDouglasPeucker = [&](double _ep, InputIterator _s, 
			InputIterator _e, vector<bool>::iterator _o) -> void {
		auto sz = distance(_s, _e);
		*_o = true;

		// Base case 1 : Not enough points
		if (sz < 3) {	
			if(sz == 2) {
				advance(_o, sz-1);
  	  	*(_o) = true;
		...
		double md = 0;
		InputIterator mi = _s;
		InputIterator last = prev(_e);
		for (auto it = next(_s); it != last; it++) {
			auto d = DistanceToLine(*it, *_s, *last);
			if (d > md) {
				md = d;
				mi = it;
			}
		...
		// Base case 2 : Already simplified line
		if (md <= _ep){	
			advance(_o, sz-1);
  	  *(_o) = true;
		}
		else { 
			RamerDouglasPeucker(_ep, _s, next(mi), _o);
			advance(_o,distance(_s, mi));
			RamerDouglasPeucker(_ep, mi, _e, _o);
		}
  };

	vector<bool> rv(distance(_start, _end), false);
	RamerDouglasPeucker(_epsilon, _start, _end, rv.begin());
	return rv;
}
