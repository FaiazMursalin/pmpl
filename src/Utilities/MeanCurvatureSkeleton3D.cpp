...
BuildSkeleton(bool _curve) { 
  /*ofstream ofs("lungs-obst.off");
  ofs<<m_input;
  ofs.close();
  cout<<"total number of vertices"<<boost::num_vertices(m_input)<<endl;*/
  // validate whether the input mesh is triangulated
  if(!is_triangle_mesh(m_input)) {
    if(m_debug)
      cerr <<"Input mesh needs to be traingulated" << endl;
  ...
  if(m_debug)
    cout<<"Parameters: WH "<< mcs.quality_speed_tradeoff() 
      ...
      << " area variation " << mcs.area_variation_factor() << endl;
...
GetSkeleton() {
  ...
  for(auto pline : plines){
    ...
    epts[0] = pline.front();
    epts[1] = pline.back();
    /*size_t degree0 = degree(epts[0], m_mcs);
    size_t degree1 = degree(epts[1], m_mcs);
    if(pline.size() == 2 && (degree0 == 1 || degree1 == 1) && (degree0 > 10 || degree1 > 10))
      continue;*/
