...
BuildMedialAxis() {
 ...
 // Comparison function to check the order in the map
  auto PointCompare = [](const Point2& _p1, const Point2& _p2) -> bool {
  ...
  map<Point2, typename MedialAxisGraph::vertex_descriptor, 
    decltype(PointCompare)> vertexToDescriptorMap(PointCompare);

  // @TODO: Remove this function once the issue with the map equivalence check is
  // fixed.
  static auto DoubleCheckVertex = [&](const Point2& _p) -> 
    MedialAxisGraph::vertex_descriptor {
    for(auto vi = m_graph.begin(); vi != m_graph.end(); ++vi)
      if((_p - vi->property()).squared_length() < numeric_limits<double>::epsilon())
  ...
  double step = 0.001 * min(fabs(m_boundary[1][0]-m_boundary[0][0]),
    ...
    if(IsMedialAxisEdge(*eit)) {
      ...
      if(me != nullptr)	{
        ...
        for(size_t i = 0; i < 2; ++i) {
          auto findPt = vertexToDescriptorMap.find(fps[i]); 
          // If end point not already in the graph - add vertex
          if(findPt == vertexToDescriptorMap.end()) {
            //@TODO : Remove this double check once the issue is fixed
            eps[i] = DoubleCheckVertex(fps[i]);
            if(eps[i] == (size_t) -1) {
             eps[i] = m_graph.add_vertex(fps[i]); 
             vertexToDescriptorMap.emplace(fps[i],eps[i]);
            }
            //else 
              //cout<<"Point found in double check "<< fps[i] <<" "<<eps[i]<< endl;
          }
          else{
            eps[i] = findPt->second; 
          }
...
GetSkeleton(size_t _t) {
  ...
  for(auto eit = m_graph.edges_begin(); eit != m_graph.edges_end(); 
    ...
    auto s = g.find_vertex(eit->source())->property();
    bool reversed = false;
    if((edge.back()- s).norm() < (edge.front()- s).norm()) {
    ...
    auto edgeClearances = AnnotateSegment(eit->property());
    if(reversed)
      reverse(edgeClearances.begin(), edgeClearances.end());
    edgeMap.insert(make_pair(ed, edgeClearances));
  }
  ...
  for(auto vit = m_graph.begin(); vit != m_graph.end(); ++vit){
	auto p = vit->property();
    std::vector<ClearanceType> cl;
    // Find the clearance with respect to each adjacent edge 
    for(auto eit = vit->begin(); eit != vit->end(); ++eit) {
      cl.push_back(DistanceToSite(p, eit->property().m_site1));
	  cl.push_back(DistanceToSite(p, eit->property().m_site2));
    }
    vertexMap.insert(make_pair(vit->descriptor(),
      cl));
  }
  RemoveDegree2(g, vertexMap, edgeMap);
  WorkspaceSkeleton skeleton;
  skeleton.SetGraph(g);
  return make_tuple(skeleton, vertexMap, edgeMap);
}

MedialAxis2D::EdgeClearanceType
MedialAxis2D::
AnnotateSegment(MedialEdge& _e)	{
  MedialAxis2D::EdgeClearanceType clearances;
  ...
  for(auto it = _e.m_interpolated.begin(); it != _e.m_interpolated.end(); 
      ++it)	{
    auto cl1 = DistanceToSite(*it,_e.m_site1);
    auto cl2 = DistanceToSite(*it,_e.m_site2);
    clearances.emplace_back(make_pair(min(cl1.first,cl2.first),make_pair(cl1.second,cl2.second)));
  }
  return clearances;
}
...
// or on plane in the positive side of z axis
  static auto NearOrthogonalPlane = [&](const GMSPolygon& _poly, 
      size_t _pi = 2) -> bool {
    const double tolerance = numeric_limits<double>::epsilon(); // Tolerance for considering points near-plane.
  ...
  static auto PointInPolygon = [&](const Point2& _p, 
    ...
    for (size_t i=0; i<_v.size(); i++) {
      if(_v[i].OnLine(_p)) // if on boundary, it is not free
        return true;
			// Consider the edges left of _p (i.e, condition with x()) 
			// and intersecting the ray from _p which is parallel to x-axis 
      // (i.e., if condition on y())
			// For odd intersection point is in polygon
...
void 
MedialAxis2D::
RemoveDegree2(MedialAxis2D::SkeletonGraphType& _g, VertexClearanceMapType& _vmap, EdgeClearanceMapType& _emap) {
  bool removed;
  do {
    removed = false;
    for (auto vit = _g.begin(); vit != _g.end(); ++vit) { 
      //detect 2-node
      if (_g.get_out_degree(vit->descriptor()) +
          _g.get_in_degree(vit->descriptor()) == 2) {
        //get in and out edge
        vector<Point3d> intermediates; 
        WorkspaceSkeleton::VD src, trgt;
        src = trgt = vit->descriptor();
        WorkspaceSkeleton::ED in, out;
        bool r1=false, r2=false;
        // if number of incoming edges is greater than 0
        if (!vit->predecessors().empty()) {
          // Get the first incoming edge and the other vertex of the edge
          src = vit->predecessors()[0];
          auto pred = _g.find_vertex(src);
          // Get the first edge as the first incoming edge
          for (auto eit : *pred) {
            if (eit.target() == vit->descriptor()) {
              in = eit.descriptor();
              auto pts1 = eit.property(); 
              intermediates.insert(intermediates.end(), pts1.begin(), 
                  pts1.end());
            }
          }
          // If the vertex has only incoming edges
          // set the second incoming as reversed outgoing edge
          if (vit->predecessors().size() == 2) {
            // Get the other vertex in the second incoming edge
            trgt = vit->predecessors()[1];
            auto pred = _g.find_vertex(trgt);
            // Get the second incoming edge
            for (auto eit : *pred) {
              if (eit.target() == vit->descriptor()) {
                // This edge info is to be reversed while merging 
                r2 = true;
                out = eit.descriptor();
                auto pts2 = eit.property(); 
                intermediates.insert(intermediates.end(), pts2.rbegin() + 1, 
                    pts2.rend());
              }// end if
            }//end for
          }//end if
        }
        // When there are outgoing edges
        if (vit->size() > 0) {
          // When there are more than 1 outgoing edge 
          // consider the second outgoing edge as reversed incoming edge
          if (vit->size() == 2) {
            auto eit = vit->begin();
            ++eit;
            in = eit->descriptor();
            src = eit->target();
            auto pts1 = eit->property();
            // This edge info needs to be reversed while merging
            r1 = true; 
            intermediates.insert(intermediates.end(), pts1.rbegin(), 
                pts1.rend());
          }
          // Store the first outgoing edge as the outgoing edge to be merged
          out = vit->begin()->descriptor();
          trgt = out.target();
          auto pts2 = vit->begin()->property(); 
          intermediates.insert(intermediates.end(), pts2.begin() + 1, 
              pts2.end());
        }
        //Merge edges as new edge
        auto neweid = _g.add_edge(src, trgt, intermediates);
        //Update maps
        auto vmit = _vmap.find(vit->descriptor());
        double vcl = 0;
        // Find the vertex clearance info
        if (vmit != _vmap.end()){
		  vcl = std::numeric_limits<double>::max();
		  for(auto c : vmit->second)
			if(c.first < vcl)
			  vcl = c.first;
          _vmap.erase(vmit);
        }
        else
          cout<<"Cannot get the vertex clearance"<<endl;
        EdgeClearanceType ecl;
        size_t insertfirst = 0;
        auto emit1 = _emap.find(in);
        // Find the edge clearance info of incoming edge
        if(emit1 != _emap.end()) {
          if(r1)
            ecl.insert(ecl.end(), emit1->second.rbegin(), emit1->second.rend());
          else
            ecl.insert(ecl.end(), emit1->second.begin(), emit1->second.end());
          // If the vertex clearance is not equal to the clearance of the vertex
          // with respect to the incoming edge sites, remove the vertex
          // clearance value
          if(ecl.back().first > vcl) 
            ecl.pop_back();
          else
            insertfirst = 1;
          _emap.erase(emit1);
        }
        // Insert clearance info of outgoing edge
        auto emit2 = _emap.find(out);
        if(emit2 != _emap.end()) {
          if(r2)
            ecl.insert(ecl.end(), emit2->second.rbegin()+insertfirst, 
                emit2->second.rend());
          else
            ecl.insert(ecl.end(), emit2->second.begin()+insertfirst, 
                emit2->second.end());
          _emap.erase(emit2);
        }
        _emap.emplace(neweid, ecl);

        //Delete old edges and delete old vertex
        _g.delete_edge(in);
        _g.delete_edge(out);
        _g.delete_vertex(vit->descriptor());
        --vit;
        removed = true;
      }
    }
