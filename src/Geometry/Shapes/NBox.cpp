...
NBox::
NBox(const size_t _n) : NShape('b'), m_center(_n, 0),
...
NBox::
NBox(const std::vector<double>& _center) : NShape('b'), m_center(_center),
...
DirectedClearance(const std::vector<double>& _p, const std::vector<double>& _d) const noexcept {
  // Get the transformation of the point
  const size_t maxIndex = std::min(_p.size(), GetDimension());
  // Create a point unit distance away from the point and transform that
  ...
  for(size_t i = 0; i < m_range.size(); ++i)
    ranges.push_back(0.5 * m_range[i].Length());
  const auto& distances = LineBoxIntersection(p, _d, ranges);
