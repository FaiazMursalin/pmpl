...
class PathType {
  ...
  public:
    ...
    void Insert(int position, const vector<VID>& _vids);
...
FullCfgs(MPLibrary* _lib, const string& _lp) const {
  ...
  for(auto it = m_vids.begin(); it + 1 < m_vids.end(); ++it) {
    ...
    for(auto cit = recreatedEdge.begin(); cit + 1 != recreatedEdge.end(); ++cit) {
      if (g->GetVertex(it).IsStat("waittime")) {
          auto waittime = g->GetVertex(it).GetStat("waittime");
          cout<<"Found waittime in Path: "<<waittime<< " for " << g->GetVertex(it) << endl;
...
void
PathType<MPTraits>::
Insert(int position, const vector<VID>& _vids) {
  // the position is within bounds
  if (position < 0 || position > m_vids.size()) {
    std::string message = "Invalid position for insertion. Tried inserting at " + std::to_string(position) + ". But vector size is " + std::to_string(m_vids.size()) + ". Inserting vector size is " + std::to_string(_vids.size());
    throw std::out_of_range(message);
  }

  // insert a copy of elements from _vids into m_vids at the given position
  cout << "Before insertion: size = " << m_vids.size() << endl; 
  m_vids.insert(m_vids.begin() + position, _vids.begin(), _vids.end());  
  cout<< "Inserted " << _vids.size() <<" VIDs at " << position << ". After insertion, size = " << m_vids.size() << endl; 
  m_cfgsCached = false;
  m_lengthCached = false;
}
